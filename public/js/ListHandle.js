...
listHandle.compileFriendList = function(data,setObj){
    ...
    for(var key in data){
        ...
        currArr.forEach(function(item){
            tmpHTML += '<li class="" userId=\'uId'+item.friendId+'\' onmousedown="mouseClickUser(this,event)"> <img src="'+item.picture+'" alt="" class="am-circle" onclick="menu()"> <span>'+item.nickName+'</span></li>';
...
listHandle.msgBox = function(data,type){
    if(type == 1){
        return '<li id="users" class="" userId=\'uId'+data.friendId+'\' onmousedown="mouseClickUser(this,event)"> <img src="'+data.picture+'" alt="" class="am-circle"> <span>'+data.nickName+'</span></li>';
    }else{
